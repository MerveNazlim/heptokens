...
def _infer_dimensions(
    ...
    if "metadata" in handle and "n_events" in handle["metadata"].attrs:
        ...
        total_events = int(_resolve_h5_node(handle, object_collections[0]["inputs"][0]).shape[0])
    elif event_inputs:
...
def _find_collection(object_collections: list[dict], object_type: str) -> dict:
    for collection in object_collections:
        if collection.get("object_name") == object_type:
            return collection
    available = ", ".join(str(collection.get("object_name")) for collection in object_collections)
...
def _read_collection(
    ...
    for h5_path in inputs:
        ...
        feature_arrays.append(_slice_events(array, n_events, n_objects))
...
class AtlasEventMapDataset(Dataset):
    ...
    def _load_object(
        ...
        return {
            "csts": csts,
            "mask": mask,
            # Current VQ-VAE interfaces expect event/global inputs under "jets".
            # The selected object features are returned under "csts".
            "jets": _read_event_inputs(handle, event_inputs, n_events, fallback_jets),
        }
